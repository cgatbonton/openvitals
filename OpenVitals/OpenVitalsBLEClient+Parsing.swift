...
extension OpenVitalsBLEClient {
  ...
  func handleStandardReadValue(
    ...
    case modelNumberCharacteristicID:
      ...
      rejectUnsupportedAccessoryIfNeeded(
        characteristic: characteristic,
        metadataValue: modelNumber,
        metadataField: "model"
      )
  ...
  func whoopIdentityEvidence(
    ...
    if advertisedServices.contains(where: isWhoopService) {
      return "advertised device service"
    }
    if isWhoopPowerPackAdvertisingName(peripheral.name) || isWhoopPowerPackAdvertisingName(fallbackName) {
      return nil
    }
    if whoopCandidateIDs.contains(peripheral.identifier) {
      return "cached device service match"
    }
    ...
    if allowRememberedValidation,
      ...
      return "validated remembered device"
    }
    return nil
  }
  ...
  func isWhoopName(_ name: String?) -> Bool {
    ...
    if name.range(of: "whoop", options: [.caseInsensitive, .diacriticInsensitive]) != nil {
      return true
    }
    return isWhoopBandAdvertisingName(name)
  }

  func isWhoopBandAdvertisingName(_ name: String?) -> Bool {
    guard let name = name?.trimmingCharacters(in: .whitespacesAndNewlines), !name.isEmpty else {
      return false
    }
    let uppercased = name.uppercased()
    guard !isWhoopPowerPackAdvertisingName(uppercased) else {
      return false
    }
    guard uppercased.hasPrefix("WBB"),
          (8...24).contains(uppercased.count) else {
      return false
    }
    let allowedCharacters = CharacterSet(charactersIn: "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
    return uppercased.unicodeScalars.allSatisfy { allowedCharacters.contains($0) }
  }

  func isWhoopPowerPackAdvertisingName(_ name: String?) -> Bool {
    guard let name = name?.trimmingCharacters(in: .whitespacesAndNewlines), !name.isEmpty else {
      return false
    }
    let uppercased = name.uppercased()
    return uppercased.contains("WIRELESS POWERPACK")
      || uppercased.contains("WIRELESS POWER PACK")
      || uppercased == "POWERPACK"
      || uppercased == "POWER PACK"
  }

  func isUnsupportedAccessoryMetadata(_ value: String?) -> Bool {
    guard let value = value?.trimmingCharacters(in: .whitespacesAndNewlines), !value.isEmpty else {
      return false
    }
    return value.range(of: "Wireless PowerPack", options: [.caseInsensitive, .diacriticInsensitive]) != nil
      || value.range(of: "PowerPack", options: [.caseInsensitive, .diacriticInsensitive]) != nil
  }

  func rejectUnsupportedAccessoryIfNeeded(
    characteristic: CBCharacteristic,
    metadataValue: String?,
    metadataField: String
  ) {
    guard isUnsupportedAccessoryMetadata(metadataValue),
          let peripheral = characteristic.service?.peripheral else {
      return
    }
    unsupportedAccessoryIDs.insert(peripheral.identifier)
    record(
      level: .warn,
      source: "ble",
      title: "unsupported_accessory.detected",
      body: "\(metadataField)=\(metadataValue ?? "unknown") id=\(peripheral.identifier.uuidString)"
    )
    rejectNonWhoopPeripheral(
      peripheral,
      reason: "unsupported_powerpack_accessory",
      fallbackName: metadataValue,
      disconnect: true
    )
  }
  ...
  func commandWriteAuthenticationFailureMessage(for error: Error) -> String? {
    ...
    if message.contains("authentication is insufficient")
  ...
  func handleCommandWriteFailure(_ error: Error, characteristic: CBCharacteristic) {
    guard commandCharacteristicIDs.contains(characteristic.uuid),
          let authMessage = commandWriteAuthenticationFailureMessage(for: error) else {
      return
    }
    commandWriteAuthenticationRequired = true
    commandWriteAuthenticationStatus = authMessage
    physiologyCaptureStatus = "Authentication required for command writes"
    lastPhysiologyCommandSummary = authMessage
    updateConnectionState("auth required")
    record(level: .warn, source: "ble.auth", title: "command_write.auth_required", body: "\(characteristic.uuid.uuidString) \(error.localizedDescription)")
  }

  func recordScanDiagnostic(
    id: UUID,
    name: String?,
    rssi: Int,
    advertisedServices: [CBUUID],
    ...
    let services = advertisedServices
      .map(\.uuidString)
      .joined(separator: ",")
    let diagnostic = OpenVitalsScanDiagnostic(
      id: id,
      name: Self.sanitizedWhoopDisplayName(name ?? "Nearby device"),
      rssi: rssi,
      status: status,
      services: services.isEmpty ? "no advertised services" : services,
      lastSeen: Date()
    )
    scanDiagnostics.removeAll { $0.id == id }
    scanDiagnostics.append(diagnostic)
    scanDiagnostics.sort { lhs, rhs in
      if lhs.rssi == rhs.rssi {
        return lhs.lastSeen > rhs.lastSeen
      }
      return lhs.rssi > rhs.rssi
    }
    if scanDiagnostics.count > 30 {
      scanDiagnostics.removeLast(scanDiagnostics.count - 30)
    }
  }
  ...
  func rejectNonWhoopPeripheral(
    ...
    let isPassiveScanRejection = reason.hasPrefix("scan_")
    let name = peripheral.name ?? fallbackName ?? "unknown"
    record(
      level: isPassiveScanRejection ? .debug : .warn,
      source: "ble",
    ...
    if rememberedDeviceID == peripheral.identifier && !isPassiveScanRejection {
    ...
    if activePeripheral?.identifier == peripheral.identifier {
      ...
      connectedAt = nil
      failAllDebugCommands("Connection rejected as unsupported device.")
      updateConnectionState("not a supported device")
    }
    if disconnect {
  ...
  func updateActiveDevice(_ peripheral: CBPeripheral, fallbackName: String? = nil) {
    ...
    updateActiveDeviceName(Self.sanitizedWhoopDisplayName(peripheral.name ?? fallbackName ?? rememberedDeviceName ?? "Device"))
  }
  ...
  func emitHistoricalRangeTelemetry(
    ...
    onHistoricalRangeTelemetry?(
      OpenVitalsHistoricalRangeTelemetry(
        capturedAt: Date(),
        status: status,
