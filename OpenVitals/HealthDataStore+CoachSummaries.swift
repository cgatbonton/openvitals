...
extension HealthDataStore {
  ...
  func hrvFeatureProvenanceSummary() -> String {
    ...
    let algorithm = Self.map(report, "score_result")?["algorithm_id"] as? String ?? "open_vitals.hrv.v0"
  ...
  func sleepV1ModelStatusSummary() -> String {
    ...
    let trusted = Self.intValue(status["trusted_open_vitals_sleep_nights"]) ?? 0
    return "\(state.capitalized) | \(nights) imported nights | \(trusted) trusted OpenVitals nights"
  }
  ...
  func recoveryProvidedVitalsSummary() -> String {
    ...
    return "unavailable | decoded device packet vitals required"
  }
  ...
  func packetInputSource(_ detail: String) -> HealthDataSource {
    ...
    case "step_discovery":
      return Self.boolValue(report["explicit_step_counter_found"]) == true
        ...
        : .unavailable(firstPacketAction(in: report) ?? "Device step counter not decoded")
    case "hrv":
      return Self.boolValue(report["pass"]) == true
        ? .bridgeDeviceSensor(detail)
