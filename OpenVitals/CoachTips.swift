...
enum CoachTipFactory {
  static func homeTip(healthStore: HealthDataStore, appModel: OpenVitalsAppModel) -> CoachInlineTip {
    ...
    return CoachInlineTip(
      ...
      prompt: """
      Give me today's coaching priority from my local OpenVitals context. Use readiness, sleep, recovery, strain, stress, live heart rate, and missing-data gaps. Cite the local tool outputs and keep it to one concrete next action.
  ...
  static func metricTip(
    ...
    default:
      ...
      return CoachInlineTip(
        ...
        prompt: "Explain my \(route.title.lowercased()) page using the local OpenVitals context. Cite the tool outputs and call out stale or missing data.",
        systemImage: "sparkles",
        tint: snapshot.tint
      )
    }
  ...
  static func sleepTip(healthStore: HealthDataStore, ble: OpenVitalsBLEClient) -> CoachInlineTip {
  ...
  private static func strainTip(healthStore: HealthDataStore, appModel: OpenVitalsAppModel) -> CoachInlineTip {
  ...
  private static func stressTip(healthStore: HealthDataStore, appModel: OpenVitalsAppModel) -> CoachInlineTip {
