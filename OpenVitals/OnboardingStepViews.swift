...
struct OnboardingHeader: View {
  ...
  var body: some View {
    HStack(alignment: .top, spacing: 12) {
      OpenVitalsLogoMark(size: 46, cornerRadius: 9)
...
struct OnboardingProfileStep: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 18) {
      Text("These basics help OpenVitals calculate your local metrics.")
        .font(.body)
        .foregroundStyle(.secondary)
...
struct OnboardingConnectStep: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      if !ble.discoveredDevices.isEmpty {
        VStack(alignment: .leading, spacing: 10) {
          OnboardingSectionLabel("Choose your device")
          VStack(spacing: 8) {
  ...
  private var connectHeading: String {
    if canVerify {
      return "Wearable is connected"
    }
    if connected {
      return "Reading device data"
    }
    if hasDiscoveredStraps {
      return "We found a compatible device nearby"
    }
    if searching {
      return "Looking for your wearable"
    }
    return "Pair your wearable"
  }

  private var connectBody: String {
    if canVerify {
      return "Finish setup to start using OpenVitals with this device."
    }
    if connected {
      return "Keep the device close while OpenVitals confirms it can read data."
    }
    if hasDiscoveredStraps {
      return "Select the device you want to use with OpenVitals."
    }
    if searching {
      return "Keep Bluetooth on and keep the device close to this phone."
    }
    return "Take the device off your wrist, keep it nearby, then start pairing."
  }
...
struct OnboardingConnectActionBar: View {
  ...
  private var primaryTitle: String {
    ...
    if hasDiscoveredStraps {
      return "Connect selected device"
    }
    return searching ? "Searching..." : "Find my wearable"
  }
